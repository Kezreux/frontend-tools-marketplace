# Component library

Curated preset components for the `kezreux-frontend` plugin. Each
preset is **theme-agnostic** — it uses only semantic tokens
(`bg-background`, `text-foreground`, `bg-primary`, etc.) so it renders
correctly in whichever theme is active.

## Install

In a project with a theme already installed (`/theme set <name>`):

```
/component <category> list        # list components in a category
/component <category> <name>      # install one
/component preview <name>         # show the source without installing
/component list                   # list every category + counts
```

## Categories

| Category | Components | Subcategories | What's in it |
| --- | --- | --- | --- |
| `navbar`    |  5 | — | Top navs (simple, with auth, with search), vertical sidebar, mobile menu drawer |
| `hero`      |  5 | — | Simple, minimal, split (with visual), with-form (email signup), with-stats |
| `form`      | 10 | `auth` (4), `feedback` (2), `settings` (2), `search` (2) | Login, signup, password reset, 2FA, contact, feedback survey, profile, account settings, search box, filter panel |
| `feedback`  |  5 | — | Modal, Toast, AlertBanner, EmptyState, LoadingSpinner |
| `marketing` |  5 | — | PricingTable, FeatureGrid, CTABanner, FAQAccordion, LogoCloud |
| `data`      |  5 | — | StatCard, DataTable, UserList, KeyValueList, Timeline |
| `layout`    |  4 | — | CenteredPage, SidebarLayout, DashboardGrid, FullPageLayout |

Categories use **optional 2-level nesting**. Flat categories keep their
`.tsx` files directly under the category dir. Categories that benefit
from grouping (`form`) split into subcategory folders. The generator
and `/component` command handle both shapes.

**Total: 39 components across 7 categories.**

## Authoring a new component

1. Add the metadata header (5 tags — see `CATALOG.md`).
2. Export the component and its `Props` interface.
3. Export `demos: Record<string, <Props>>` with at least a `default` entry
   (see `CATALOG.md` for the exact contract).
4. Run `npm run build:catalog` at the marketplace root.
5. Commit `catalog.json` alongside your component.

```tsx
// @component: <PascalCaseName>
// @category: <lowercase-category>
// @description: <one-line summary>
// @keywords: comma, separated, intent, terms
// @complexity: simple | medium | complex

import { useState } from "react";

export interface <Name>Props { ... }

export function <Name>(props: <Name>Props) { ... }

export const demos: Record<string, <Name>Props> = {
  default: { /* sample props */ },
};
```

The `/component` command discovers presets by globbing the directories —
no manual registry to update. The `catalog.json` file (generated by
`npm run build:catalog`) is what downstream consumers (gallery websites,
docs) consume — see `CATALOG.md` for its schema.

## Token-only contract

Every preset must use semantic tokens. The five things to never do:

1. No hardcoded hex (`#fff`) — use `bg-background`, `text-foreground`.
2. No literal `bg-white` / `bg-black` — breaks dark mode.
3. No Tailwind arbitrary px (`p-[13px]`) — use the spacing scale.
4. No inline color styles — use Tailwind classes.
5. No external imports outside `react`. Optional internal `cn()` is fine
   but components also work with raw template-literal classNames.
